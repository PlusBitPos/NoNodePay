...
<script>
const watchAddress = "<?php echo $_SESSION['payTo']; ?>";
...
insight.on('tx', function(data) {
  const amount = data.vout.reduce(
    function (accum, item) {
      return accum + (
        Object.keys(item)[0] === watchAddress
  ...
  if (amount > 0) {
    const message = '<a href="https://ilcoinexplorer.com/tx/' + data.txid + '">' +
      'You paid ' + amount / 100000000 + ' ILC, click for TX details' +
      '</a><br>';
...
<div id="viewCart">
  ...
  <div id="payAmt"><b>Amount Due: <?php echo $_SESSION['orderCost']; ?> ILC</b><br>
  Remit payment to the below address to complete your order. <br>
  <div id="qrcode"></div>
  <script type="text/javascript">
  new QRCode(document.getElementById("qrcode"), "ilcoin:<?php echo $_SESSION['payTo']; ?>?amount=<?php echo $_SESSION['orderCost']; ?>");
  </script>
  <br>
